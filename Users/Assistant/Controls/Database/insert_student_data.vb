...
Public Class insert_student_data
    ...
    Private Sub insert_student_data_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub db_select_SelectedIndexChanged(sender As Object, e As EventArgs) Handles db_select.SelectedIndexChanged
        ...
        cmd.CommandText = "select count(name) as max from sys.tables where name like 'stu_personal%';"
        rd = cmd.ExecuteReader
        rd.Read()
        Dim count As Integer = rd("max")
        rd.Close()

        cmd.CommandText = "select name from sys.tables where name like 'stu_personal%';"
        rd = cmd.ExecuteReader
        If rd.HasRows = True Then
            For i As Integer = 0 To count - 1
                rd.Read()
