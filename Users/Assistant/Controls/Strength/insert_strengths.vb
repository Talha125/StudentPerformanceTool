...
Public Class insert_strengths

    Private Sub insert_strengths_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub sem_SelectedIndexChanged(sender As Object, e As EventArgs) Handles sem.SelectedIndexChanged
        ...
        cmd.CommandText = "select count(name) as max from sys.tables where name like 'sem" & sem.SelectedItem & "_stu%';"
        rd = cmd.ExecuteReader
        rd.Read()
        Dim count As Integer = rd("max")
        rd.Close()

        cmd.CommandText = "select name from sys.tables where name like 'sem" & sem.SelectedItem & "_stu%';"
        rd = cmd.ExecuteReader
        If rd.HasRows = True Then
            For i As Integer = 0 To count - 1
                rd.Read()
